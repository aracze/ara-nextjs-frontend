...
export function Header({ pages, header }: { pages: Page[], header?: GlobalHeader }) {
                        ...
                        <Link href="/" className="flex items-center">
                            {logo.svgCode ? (
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{ __html: logo.svgCode.replace(/fill="#fff"/g, 'fill="white"').replace(/fill="#[a-f0-9]{6}"/gi, 'fill="white"') }}
                                />
                            ) : logo.image && (
                                <Image
                                    src={new URL(logo.image.url, process.env.NEXT_PUBLIC_STRAPI_BASE_URL || 'http://localhost:1337').toString()}
